        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" wire:loading.class="opacity-50">
            @foreach($favorites as $fav)
                ...
                <div wire:key="fav-item-{{ $fav->id }}" 
                     class="group bg-beige border shadow-md border-gray-200 rounded-xl overflow-hidden h-full flex flex-col relative">

                    {{-- LINK UTAMA --}}
                    <a href="{{ route('event.show', $event->id) }}" class="absolute inset-0 z-10"></a>

                    {{-- FOTO EVENT --}}
                    <div class="relative h-48 w-full overflow-hidden bg-gray-100">
                        <div class="absolute inset-0">
                            <img src="{{ $imageUrl }}" class="w-full h-full object-cover blur-xl scale-110 opacity-50">
                        </div>
                        <div class="relative h-full w-full flex justify-center items-center z-0">
                            <img src="{{ $imageUrl }}" alt="{{ $event->name }}" class="h-full w-auto object-contain shadow-sm">
                        </div>
                    </div>

                    {{-- KONTEN --}}
                    <div class="p-3 flex flex-col flex-1 relative z-20 pointer-events-none">
                        
                        {{-- HEADER: Judul & Tombol Favorit --}}
                        <div class="flex justify-between items-start gap-2">
                            {{-- Judul --}}
                            <h3 class="text-md font-bold text-[#172a39] line-clamp-2 leading-snug">
                                {{ $event->name }}
                            </h3>
                            {{-- TOMBOL FAVORIT --}}
                            <div class="pointer-events-auto flex-shrink-0 pt-0.5 pr-2" @click.stop>
